document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleMove(x, y) { 
        ...
        if (diorama) diorama.style.transform = `rotateX(${rotX}deg) rotateY(${rotY}deg)`;
    }
    ...
    cat1.addEventListener('mouseenter', () => {
        if (cat1) {
            for (let i = 0; i < 2; i++) createHoverHeart(cat1);
        }
    });
    ...
    function createClickHeart(x, y) {
        ...
        setTimeout(() => { const destX = (Math.random() - 0.5) * 300; const destY = (Math.random() - 0.5) * 300 - 150; heart.style.transform = `translate(${destX}px, ${destY}px) rotate(${(Math.random() - 0.5) * 720}deg) scale(0)`; heart.style.opacity = '0'; }, 10);
    ...
    function typeWriter(text, element, i = 0) {
        ...
        if (i < text.length) {
            ...
            if (element.parentElement) {
                element.parentElement.style.borderRight = 'none';
            }
        }
    }
